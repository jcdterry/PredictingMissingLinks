...
try(setwd("C:/Users/pemb4504/Dropbox/0DrosPostdoc/PredictionOfMissingLinks"))
try(setwd("C:/Users/Chris/Dropbox/0DrosPostdoc/PredictionOfMissingLinks"))
...
save(EmpData, file='Outputs/EmpDataRedo') # Some redone, so now called
```
...
Mod_Names <- c('Coverage Deficit','Sample Size','Block',
               'Connectance',  'Latent-Trait', 'Matching-Centrality'  )
...
ggplot(tmpEmp, aes(y= AUC,
  ...
  #ggtitle('Model Perfomance on Emp Data')+
...
tmpEmp%>%
  ...
  # geom_point(aes(x=Value, y= AUC,col= PredictiveModel))+
  scale_colour_manual(values = Colours, name = 'Predictive Model')+
  #geom_rug(aes(x=Value, y= AUC),sides = 'b')+
...
save(result, file = 'Outputs/Artificial_result_redo')
```

## Secondary Data Processing

```{r}
load('Outputs/Artificial_result_redo')
...
save(NicheModelResults, file='Outputs/NicheModelResults2000redo')
```
...
load('Outputs/Artificial_result_redo')
load('Outputs/NicheModelResults2000redo')
...
Fig2Legend <- get_legend(EmpiricalBoxplot+ theme(legend.text = element_text(size = 18),
                                                 legend.title = element_text(size = 20)))

Fig2Top<-plot_grid(ArtificialBoxplot,
                   EmpiricalBoxplot+
                     guides(fill=FALSE)+
                     theme(strip.background = element_rect(fill = 'grey95')),
                   rel_widths = c(1,2),
                   labels = c('a) Artificial Dataset', 'b) Empirical Dataset'), 
                   scale = 0.9)
...
tmpNM%>%
  ...
  ggplot()+
  geom_smooth(aes(x=Value, y= AUC,col= PredictiveModel),se= FALSE)+
  #geom_point(aes(x=Value, y= AUC,col= PredictiveModel), size=0.1)+
  scale_colour_manual(values = Colours, name = 'Predictive Model')+
  #geom_rug(aes(x=Value, y= AUC),sides = 'b')+
  facet_grid(Dataset~`Network Property`, scales = 'free_x')+
  theme(axis.title.x = element_blank(),
...
ST_PR_Art <- map_df(result, function(list){
  ...
  CORS <- data.frame('Coverage Deficit'= cor(ST_PR$TrueStrength, ST_PR$C_defs_missed, method = 'spearman'),
                     'Connectance'= cor(ST_PR$TrueStrength, ST_PR$C_missed, method = 'spearman'),
...
ggsave('Images/FigureS5.png', width=8, height=7, dpi=300)
ggsave('Images/FigureS5.pdf', width=8, height=7)
...
ggsave('Images/Figure4.png', height=6, width=10, dpi=300)
ggsave('Images/Figure4.pdf', height=6, width=10)
...
# 
# 
#   mutate(Label1 =paste0('Enhanced \nCorr:', str_pad(EnhancedCor,8, side = 'both'),'\n', 
#                        'Bias:', str_pad(EnhancedBias,8, side = 'both'), '\n',
#                        'Mean Error:', str_pad(EnhancedMeanErr,8, side = 'both'),'\n'),
#          Label2 = paste0('Observed\n',
#                          str_pad(ObservedCor,8, side = 'both'),'\n',
#                          str_pad(ObservedBias,8, side = 'both'),'\n',
#                          str_pad( ObservedMeanErr,8, side = 'both'),'\n' ))
...
cowplot::plot_grid(PlotFit(demoweb, 'C', title = 'a) Connectance Model')+ THEME,
                   ...
                   PlotFit(demoweb, 'SBM', OrderBy = 'SBM', title = 'e) Block')+ THEME,
                   PlotFit(demoweb,c('SBM','C_def'), Combine = '+', OrderBy = 'SBM', 
                           title = 'f) Block and Coverage-Deficit Model')+ THEME,
                   ncol = 3 )
...
# cowplot::plot_grid(PlotFit(demoweb, 'C', title = 'a) Connectance Model')+ THEME,
#                    PlotFit(demoweb, 'M',title = 'b) Latent-Trait Model')+THEME,
#                    PlotFit(demoweb, 'C_def', addDots = 'Size',
#                            title = 'c) Coverage-Deficit Model', RemoveTP = TRUE)+THEME,
#                    PlotFit(demoweb, 'SBM',  title = 'd) Stochastic Block Model')+THEME,
#                    PlotFit(demoweb, 'B', title = 'e) Matching-Centrality Model')+THEME,
#                    PlotFit(demoweb,c('SBM','C_def'), Combine = '+', OrderBy = 'Degree',
#                            title = 'f) SBM and Coverage-Deficit Model')+THEME,
#                    ncol = 3 )
# 
# ggsave('Images/ExamplePlot2.png', width = 12, height=8, dpi = 300)
# ggsave('Images/ExamplePlot2.pdf', width = 12, height=8)
...
# Example Plots of Predictive Model Fit (v2)
...
cowplot::plot_grid(PlotFit(demoweb, 'C', OrderBy = 'Degree',
                           title = 'a) Connectance Model', RemoveTP = TRUE)+ THEME,
                   PlotFit(demoweb, 'M', OrderBy = 'Degree',
                           title = 'b) Latent-Trait Model', RemoveTP = TRUE)+ THEME,
                   PlotFit(demoweb, 'C_def', OrderBy = 'Degree', RemoveTP = TRUE,
                           title = 'c) Coverage-Deficit Model')+ THEME,
                   PlotFit(demoweb, 'B', OrderBy = 'Degree',
                           title = 'd) Matching-Centrality Model', RemoveTP = TRUE)+ THEME,
                   PlotFit(demoweb, OrderBy = 'Degree','SBM',
                           title = 'e) Block Model', RemoveTP = TRUE)+ THEME,
                   PlotFit(demoweb,c('SBM','C_def'),  OrderBy = 'Degree',Combine = '+',
                           title = 'f) Block and Coverage-Deficit Model', RemoveTP = TRUE)+ THEME,
                   ncol = 3 )

ggsave('Images/Figure1v2.png', width = 12, height=8, dpi = 300)
ggsave('Images/Figure1v2.pdf', width = 12, height=8)
